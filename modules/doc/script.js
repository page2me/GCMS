function inintDocCat(id) {
	var patt = /icon-(expand|collapse)/;
	var _doClick = function() {
		if (patt.test(this.className)) {
			this.className = this.className == 'icon-collapse' ? 'icon-expand' : 'icon-collapse';
		}
	};
	forEach($G(id).elements('span'), function() {
		callClick(this, _doClick);
	});
}
function inintDocView(id, cat) {
