<?php
/*
 * @filesource personnel/models/admin/settings.php
  ...
  public function save()
  {
    ...
    if (self::$request->initSession() && self::$request->isReferer() && $login = Login::isMember()) {
      if ($login['email'] == 'demo') {
        ...
        // รับค่าจากการ POST
        $save = array(
          'image_width' => max(75, self::$request->post('image_width')->toInt()),
          'image_height' => max(75, self::$request->post('image_height')->toInt()),
          'can_write' => self::$request->post('can_write', array())->toInt(),
          'can_config' => self::$request->post('can_config', array())->toInt()
        );
        // โมดูลที่เรียก
        $index = \Index\Adminmodule\Model::get('personnel', self::$request->post('id')->toInt());
        if ($index && Gcms::canConfig($login, $index, 'can_config')) {
          ...
          $ret['location'] = self::$request->getUri()->postBack('index.php', array('module' => 'personnel-settings', 'mid' => $index->module_id));
        } else {
          $ret['alert'] = Language::get('Unable to complete the transaction');
        }
      }
