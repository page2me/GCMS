<?php
/*
 * @filesource maxsite.php
 * @link http://www.kotchasan.com/
 * @copyright 2016 Goragod.com
 * @license http://www.kotchasan.com/license/
 */

namespace Index\Maxsite;
  ...
  public static function import($db)
  {
    ...
    if (Upgrade::tableExists($db, $prefix.'_member')) {
      foreach ($db->customQuery("SELECT * FROM `{$prefix}_member` WHERE `email` != '' ORDER BY `id`;") as $item) {
        ...
        if (preg_match('/^([^\s]+)\s(.*)/', $item->name, $match)) {
          $fname = $match[1];
          $lname = $match[2];
        } else {
          $fname = $item->name;
        ...
        $users[$item->email] = array(
          'id' => $item->id,
          'email' => $item->email,
          'password' => md5($item->password.$item->email),
          'fname' => $fname,
          'lname' => $lname,
